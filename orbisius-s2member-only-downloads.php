...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
// Installation: 1) Paste the code at the end of the main plugin file. 2) Rename the class name i.e. put some prefixes
$orb_updater_obj = new orbisius_s2member_only_downloads_updater();

add_filter('pre_set_site_transient_update_plugins', array($orb_updater_obj, 'check_for_update')); // Hook into the plugin update check
add_filter('plugins_api', array($orb_updater_obj, 'get_info'), 10, 3); // Hook into the plugin details screen
//add_filter('plugins_api_result', array($orb_updater_obj, 'show_api_response'), 10, 3); // dbg

class orbisius_s2member_only_downloads_updater {
    private $updater_api_end_point = null;
    private $user_agent = 'Orbisius Plugin Updater/1.0 (+http://orbisius.com)';

    public function __construct() {
        $this->updater_api_end_point = !empty($_SERVER['DEV_ENV'])
                ? 'http://127.0.0.1/projects/wp-plugin-updater-book/'
                : 'http://club.orbisius.com/wpu/';
    }

    /**
     * Shows info that comes as a reply to the API.
     *
     * @param type $res
     * @param type $action
     * @param type $args
     * @return type
     */
    public function show_api_response($res, $action, $args) {
        echo "<h3>Plugin Update Result</h3><pre>";
        print_r($res);
        echo "</pre>";
     ...
     * If yes then the $transient array is updated.
    ...
    public function check_for_update($transient) {
        ...
        $args = array(
            'action' => 'plugin.check_update',
            'plugin_name' => $plugin_name,
            'version' => $transient->checked[$plugin_name],
        );

        // Send request checking for an update
        $response = $this->make_request($args);

        // If response is false, don't alter the transient
        if ((false !== $response)
                && !is_wp_error($response)
                && !empty($response->plugin_name)) { // if there isn't an update we'll not be returning plugin_name
            $transient->response[$plugin_name] = $response;
        }
    ...
    public function get_slug() {
        $plugin_name = plugin_basename(__FILE__);
        $plugin_slug = dirname($plugin_name); // plugin

        return $plugin_slug;
    }

    /**
     * Queries the API to see what info it can get for the current plugin.
     *
     * @param string $def
     * @param string $action
     * @param string $args
     * @return reject array or WP_Error
     */
    public function get_info($def, $action, $args) {
        $plugin_name = plugin_basename(__FILE__);
        $plugin_slug = $this->get_slug(); // plugin's folder
    ...
    /**
     * Sends a request to the alternative API, return an object
     * @param type $args
     * @return object returned from the API or WP_Error in case of an error
     */
    public function make_request($args) {
        ...
        $args = array_merge($sys_arg, $args);
        $request = wp_remote_post($this->updater_api_end_point, array('body' => $args, 'user-agent' => $this->user_agent));

        if (is_wp_error($request) || wp_remote_retrieve_response_code($request) != 200) {
            $res = new WP_Error('plugins_api_failed', __('An Unexpected HTTP Error occurred during the API request.</p>
                <p><a href="?" onclick="document.location.reload(); return false;">Try again</a>'),
                        is_object($request) ? $request->get_error_message() : 'n/a: error');
        } else {
